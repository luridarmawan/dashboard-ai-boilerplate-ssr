...
Masuk dengan `BOOTSTRAP_ADMIN_EMAIL` / `BOOTSTRAP_ADMIN_PASSWORD` dari `.env`. Halaman `/` menyajikan halaman depan bawaan (landing modul `Example` ada di `/example` dan bisa dijadikan isi `/` dari Pengaturan); dashboard ada di `/dashboard`.
...
- **Mengganti landing page** — **Pengaturan → Aplikasi → Halaman depan (anonim)** (`app.landing_route`), berlaku seketika tanpa restart; route yang tidak ada ditolak saat disimpan. Kosongkan kolomnya untuk kembali ke `LANDING_ROUTE` di `.env` (fallback bootstrap, baku `/`). Isi `/` dengan **halaman depan bawaan** — landing korporat generik milik core (hero, lima prinsip produk, fitur platform, status instalasi + modul yang terpasang **dan aktif** untuk tenant itu, dari `GET /v1/version` disaring dengan `GET /v1/module/enabled`, alur developer, CTA) — dengan mengisi `/`. Bila `/` menampilkan halaman depan bawaan padahal route lain yang dipilih, route pilihan itu tidak bisa dirender — modulnya nonaktif untuk tenant itu, atau routenya sudah tidak ada; alasannya tercatat sebagai peringatan di log web, dan `curl -I /` menunjukkan header `x-landing-route` saat forwarding berhasil. Modul mana pun boleh menyumbang halaman publik (`public.ts`).
- **Mengganti judul & footer** — `APP_LANDING_TITLE`, `APP_LANDING_LEAD`, dan `APP_FOOTER_TITLE` di `.env` / `.env.prod` menimpa `landing.title`, `landing.lead`, dan `shell.footer` di semua bahasa; `APP_LANDING_TITLE` sekaligus menjadi judul dokumen OpenAPI di `/docs` **dan nama merek di email keluar** (header, baris footer, dan subjek) — di situ ia menimpa `app.name` dari Pengaturan. Nama, logo, dan kalimat pembuka per tenant tetap di **Pengaturan → Aplikasi** — `app.landing_lead` yang terisi menimpa `APP_LANDING_LEAD` untuk tenant itu. Panel kanan layout masuk `split-hero` juga memakai dua nilai itu: `APP_LANDING_LEAD` sebagai judul besar, `APP_FOOTER_TITLE` sebagai baris di bawahnya.
- **Mengganti provider AI** — **Pengaturan → AI**: base URL OpenAI-compatible, kunci API (tidak pernah dikirim ke klien), model. Beberapa provider/model sekaligus dengan harga per model: **Penyedia AI** (`/m/ai/providers`), dipilih per percakapan — tiap model bisa **diuji sendiri** (reasoning/tools/endpoint kerap berbeda antar model di balik satu base URL); biaya dan token per hari/model/pengguna di **Analitik AI**. Rincian di [`docs/AI.md`](./docs/AI.md). Provider tiruan untuk pengembangan: `bun run ai:mock`.
- **Menemukan dokumentasi dari dalam aplikasi** — grup menu **Dokumentasi** (paling bawah, di bawah Pemantauan) berisi **Panduan izin** (`/permissions`, izin `group.read`: arti `sumberdaya.aksi`, wildcard, dan registry hidup instalasi ini) dan **Dokumentasi API** (`/docs`, izin `config.read`: Scalar di atas OpenAPI yang dibangun dari skema rute, dibuka di tab baru). Pembaca yang tidak memegang izinnya tidak melihat grupnya sama sekali.
...
- **Impersonasi (izin `user.impersonate`)** — dari daftar pengguna (tombol ikon di sebelah **Ubah**) atau dari halaman detail, **Masuk sebagai …** membuka dasbor seperti yang dilihat pengguna itu selama 1 jam: cookie kedua `crk_impersonate` di samping sesi admin sendiri, penanda merah berisi nama pengguna yang diperankan di menu akun (kanan atas, setiap halaman) dengan tombol **Berhenti** di dalamnya, tercatat di audit (`user.impersonate_start`/`_stop`). Selama impersonasi kata sandi dan 2FA pengguna tidak bisa diubah. Superadmin menjangkau semua akun; pemegang izin lain hanya anggota tenant aktifnya, dan tidak pernah pengguna yang memegang izin di luar miliknya sendiri.
...
- **Satu sesi per pengguna** — `SINGLE_LOGIN_ENABLE=true` di `.env` (baku `false`) membuat login yang berhasil mencabut semua sesi lain milik akun itu di server, jadi peramban atau perangkat sebelumnya keluar sendiri pada permintaan berikutnya — bukan sekadar kehilangan cookie. Berlaku untuk login kata sandi, langkah kedua 2FA, masuk lewat Google, dan penerimaan undangan; jumlah sesi yang dicabut tercatat di audit `auth.login` (`singleLoginRevoked`).
...
- **Email** — isi `SMTP_*` dan `MAIL_FROM_*` di `.env` untuk bootstrap, atau di **Pengaturan → Email** (nilai yang terisi di Pengaturan menang per kolom). Semua email lewat outbox dengan retry; tanpa SMTP di luar production, email dicetak ke log. `MAIL_BCC` (opsional, `.env` saja) menambahkan BCC ke setiap email keluar — arsip atau kotak kepatuhan. Uji SMTP dari Pengaturan → Email ikut tercatat di `/outbox`, dan setiap baris di sana (gagal maupun terkirim) bisa dikirim ulang. Kolom **Keterlibatan** di `/outbox` menunjukkan dibuka / diklik / ditindaklanjuti; pelacakan buka & klik dinyalakan per tenant lewat **Pengaturan → Email → Lacak pembukaan & klik email** (baku mati), sedangkan "ditindaklanjuti" selalu tercatat dari token sekali pakai yang dipakai. Uji kredensial `.env` tanpa database: `bun run mail:test --to anda@contoh.id`.
...
- **Deploy** — [`docs/DEPLOY.md`](./docs/DEPLOY.md): `compose.prod.yml` dengan Caddy (TLS otomatis), `--scale api=N`, backup harian, restore satu perintah. Upgrade **tanpa downtime**: `sh deploy/upgrade.sh`; cek kesiapan kapan saja: `dc run --rm preflight`; tanpa Docker: `bun start` (satu port, §8e) — dijaga systemd user service lewat `bun run systemd:install` (§8f) — atau unit systemd ber-root di `deploy/systemd/` (§8c).
...
Lisensi: **MIT** — lihat [`LICENSE`](./LICENSE); `"license": "MIT"` di setiap `package.json`. Kontribusi: gaya kode diatur Biome (`bun run lint`). Komentar kode berbahasa Inggris, dokumen dan teks antarmuka berbahasa Indonesia.
