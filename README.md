...
- **Email** — isi `SMTP_*` dan `MAIL_FROM_*` di `.env` untuk bootstrap, atau di **Pengaturan → Email** (nilai yang terisi di Pengaturan menang per kolom). Semua email lewat outbox dengan retry; tanpa SMTP di luar production, email dicetak ke log. `MAIL_BCC` (opsional, `.env` saja) menambahkan BCC ke setiap email keluar — arsip atau kotak kepatuhan. Uji SMTP dari Pengaturan → Email ikut tercatat di `/outbox`, dan setiap baris di sana (gagal maupun terkirim) bisa dikirim ulang. Uji kredensial `.env` tanpa database: `bun run mail:test --to anda@contoh.id`.
