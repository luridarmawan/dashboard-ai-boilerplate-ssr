...
- **Impersonasi (superadmin)** — dari daftar pengguna (tombol ikon di sebelah **Ubah**) atau dari halaman detail, **Masuk sebagai …** membuka dasbor seperti yang dilihat pengguna itu selama 1 jam: cookie kedua `crk_impersonate` di samping sesi admin sendiri, penanda merah berisi nama pengguna yang diperankan di menu akun (kanan atas, setiap halaman) dengan tombol **Berhenti** di dalamnya, tercatat di audit (`user.impersonate_start`/`_stop`). Selama impersonasi kata sandi dan 2FA pengguna tidak bisa diubah.
