...
echo "== start mock AI provider + mock MCP server + api + web"
...
WEB_PID=$!
trap 'kill $API_PID $WEB_PID $MOCK_PID $MCP_MOCK_PID 2>/dev/null || true' EXIT
...
export MCP_MOCK_URL=http://127.0.0.1:4020/mcp
