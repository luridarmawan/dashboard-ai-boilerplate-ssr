...
- **Lanjutan 2026-09-25 (ROADMAP §8 #54):** resolver pertama di `/resolve` kini **tabel aturan** `modules/Example/trap.ts` (pola regex → hit bertipe, diuji unit), baru database. Dua aturan demo `/category-<slug>` dan `/<category>/<item>` dengan halaman yang memperlihatkan mekanismenya — pola yang akan disalin modul e-commerce untuk mempertahankan URL situs lama.
- **Lanjutan 2026-09-26 (ROADMAP §8 #55):** setelan bertipe **`not_found_route`**, bukan `public_route`: hanya halaman berflag `notFound: true` di `public.ts` yang ditawarkan (label `Example — /resolve`), landing/katalog ditolak saat simpan dan diabaikan hooks. Alasannya dari uji staging: `/example` sebagai penangkap membuat setiap URL asing menjadi landing 200, sedangkan ekspektasi pemilik "selain URL yang dikenal modul, tetap 404".
