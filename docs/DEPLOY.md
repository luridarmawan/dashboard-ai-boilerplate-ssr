...
- **Apache di depan `bun start`**: `mod_proxy` menulis `X-Forwarded-For` dan `X-Forwarded-Host`, tetapi **tidak** `X-Forwarded-Proto`, dan tanpa `ProxyPreserveHost On` header `Host` yang sampai ke gateway adalah `127.0.0.1:3000`. Gateway lalu menebak skema dari `X-Forwarded-Host` (bila ada) yang dicocokkan dengan `APP_ORIGIN`, jadi vhost polos tetap jalan — tetapi pakailah [`deploy/apache.conf.example`](../deploy/apache.conf.example) apa adanya: `RequestHeader set X-Forwarded-Proto "https"` (**jangan** ikut menyetel `X-Forwarded-Host`/`X-Forwarded-For` — `mod_proxy` menambahkannya sendiri dan menggabungkan dengan koma, dan web menjawab 400 untuk nilai ganda itu), `ProxyPreserveHost On`, dan `flushpackets=on timeout=600` pada `ProxyPass` (tanpa itu jawaban chat AI yang di-stream bisa tertahan atau terputus di 30–120 s). Jangan menyalakan `ProxyErrorOverride On`: Apache mengganti **setiap** body 4xx/5xx dari aplikasi dengan halaman galatnya sendiri, sehingga amplop JSON (`no_api_key`, `csrf_failed`, kuota habis) hilang dan UI hanya bisa menampilkan pesan generik.
