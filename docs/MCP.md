...
| | |
|---|---|
| **UI** | **Asisten AI → Server MCP** (`/m/ai/mcps`): tambah server (nama, kode, transport, URL, header rahasia), **Uji & muat tool**, aktif/nonaktif, hapus. Butuh `ai.mcp.read` untuk melihat, `ai.mcp.manage` untuk mengubah. Tombol ujinya berjalan lewat `fetch` (route `POST /m/ai/mcps/:id/test` di web) sehingga status, jumlah tool, dan tabelnya berganti **tanpa muat ulang**, dengan ikon berputar selama koneksi berjalan — pada server yang sudah `ok` ia berbunyi **Uji ulang & muat ulang tool**. Di sebelah kanannya ada **pencarian tool** (nama, nama kawat, deskripsi) yang menyaring saat diketik dan **filter status** (Semua / Hanya aktif / Hanya nonaktif). Tiap baris tool punya **kotak centang** yang menentukan apakah tool itu ditawarkan ke asisten — bawaan aktif, kotak di kepala tabel memilih/melepas semua baris yang tampil, dan setiap perubahan **tersimpan langsung** lewat `fetch` (route `POST /m/ai/mcps/:id/tools` di web) tanpa tombol simpan; tool yang dinonaktifkan tetap nonaktif setelah uji ulang. Semuanya punya dasar tanpa JavaScript: form action `?/test`, field `GET ?q=` dan `?status=`, dan form `?/tools` dengan tombol Terapkan di `<noscript>` (L-22) |
| **API** | `GET/POST /v1/m/ai/mcps` · `GET/PUT/DELETE /v1/m/ai/mcps/:id` · `POST /v1/m/ai/mcps/:id/test` (terhubung, memuat `tools/list`, menyimpannya ke `ai_mcp_tools`, mencatat status) · `PUT /v1/m/ai/mcps/:id/tools` (`{ ids, enabled }`: aktif/nonaktifkan sekumpulan tool milik server itu, jawaban = daftar tool terkini; audit `ai.mcp.tools`) |
...
| **Tabel** | `ai_mcps` (per tenant, kode unik per tenant, header disimpan sebagai rahasia dan selalu ditampilkan `***`) dan `ai_mcp_tools` (tool hasil discovery, skema argumen mentah dari server, `enabled` per tool — hanya yang aktif masuk registry; uji ulang mengganti barisnya tetapi mempertahankan pilihan berdasarkan nama kawat) |
...
Bukti: `modules/AI/test/integration/mcp-client.test.ts` — server MCP nyata (SDK resmi) di dalam proses yang menuntut header `x-api-key`: registrasi → uji → tool tersimpan dengan nama kawat yang sah; tool muncul di `/v1/tools` hanya bagi pemegang `ai.mcp.use` dan hanya di tenant pemilik; `POST /v1/tools/call` meneruskan ke server jauh dengan header rahasia; loop chat AI memanggil tool eksternal seperti tool modul; menonaktifkan **satu tool** menyembunyikan tool itu saja (dari registry, `/v1/tools/call`, dan daftar yang dikirim ke model) dan pilihannya bertahan melewati uji ulang; nonaktifkan server menyembunyikan semuanya, hapus melupakan. Sisi webnya di peramban sungguhan: `apps/web/e2e/mcp.e2e.ts`, melawan mock di atas yang dijalankan `scripts/ci/m1-proof.sh` — dengan JavaScript (tidak ada muat ulang di seluruh uji, uji ulang, uji gagal, dan centang/lepas tool — termasuk pilih semua dan pilih semua di bawah pencarian) dan tanpa JavaScript (form action yang sama tetap menghubungkan, menyimpan, dan menerapkan pilihan tool).
