...
| | |
|---|---|
| **UI** | **Asisten AI → Server MCP** (`/m/ai/mcps`): tambah server (nama, kode, transport, URL, header rahasia), **Uji & muat tool**, aktif/nonaktif, hapus. Butuh `ai.mcp.read` untuk melihat, `ai.mcp.manage` untuk mengubah. Tombol ujinya berjalan lewat `fetch` (route `POST /m/ai/mcps/:id/test` di web) sehingga status, jumlah tool, dan tabelnya berganti **tanpa muat ulang**, dengan ikon berputar selama koneksi berjalan — pada server yang sudah `ok` ia berbunyi **Uji ulang & muat ulang tool**. Di sebelah kanannya ada **pencarian tool** (nama, nama kawat, deskripsi) yang menyaring saat diketik. Keduanya punya dasar tanpa JavaScript: form action `?/test` dan field `GET ?q=` (L-22) |
