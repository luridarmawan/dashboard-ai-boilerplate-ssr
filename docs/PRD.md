...
| `app.landing_lead` | Database (per tenant, fallback global) | kosong | Kalimat di bawah nama aplikasi (`landing.lead`: footer halaman publik, panel layout masuk `split-hero`). Terisi = menimpa `APP_LANDING_LEAD` dan teks terjemahan; kosong = `APP_LANDING_LEAD`, lalu teks bawaan |
...
| E-3 | **[P0]** Form konfigurasi **di-generate** dari metadata section + `type` field (`string`, `text`, `number`, `boolean`, `select`, `secret`, `markdown`, `route`, `public_route`, `not_found_route`, `theme`, `locale`, `timezone`, `list`). Tipe tervalidasi skema, bukan string bebas — dan tipe pula yang menentukan tambahan tampilannya (mis. `timezone` membawa jam berjalan di penutup blok fieldnya), tidak pernah nama kunci. Tata letak juga metadata: section boleh mendeklarasikan `groups` (blok berjudul, berurutan) dan field menyebut `group` + `width` (`full`/`half`/`third`); murni tampilan — nilai, validasi, dan tombol Simpan tetap per section. |
...
| ID | Kebutuhan |
|---|---|
| J-1 | **[P0]** Layanan email SMTP untuk: verifikasi email, reset password, undangan user, dan form kontak landing page (R-5). Kredensialnya bisa diuji dari **Pengaturan → Email** — tombol **Kirim email uji** (aksi section, titik perluasan 6; `POST /v1/configuration/mail/test`, izin `config.edit`) mengirim satu email langsung lewat SMTP (bukan lewat outbox J-2) dengan konfigurasi lingkup yang sedang dibuka — global atau tenant, seperti pengiriman sungguhan menyelesaikannya: setting dulu, lalu `SMTP_*`/`MAIL_*` dari `.env` (E-6). Alamat tujuannya bisa diketik, baku = akun SMTP yang berlaku. Hasilnya **dicatat ke outbox** (template `smtp-test`, status `sent`/`failed`, satu percobaan) supaya riwayat uji terlihat di `/outbox` dan bisa dikirim ulang dari sana. `MAIL_BCC` di `.env` (opsional, hanya `.env` — aturan deployment, bukan setting per tenant) menambahkan **BCC ke setiap email keluar**, baik kiriman worker outbox maupun email uji; alamatnya hanya ada di amplop, tidak di header `To`. |
| J-2 | **[P0]** Pola **outbox** — email ditulis ke tabel `outbox_email` lalu dikirim worker, dengan retry dan status. Benar-benar asinkron, tidak menahan request. Isinya dipantau dari halaman `/outbox` (grup Pemantauan, izin `mail.read`): terbaru di atas, dengan filter status/template/rentang tanggal, pencarian penerima & subjek, dan aksi **kirim ulang** / kirim sekarang untuk pemegang `mail.manage`. Kirim ulang (`POST /v1/outbox/:id/resend`) berlaku untuk baris yang gagal **maupun yang sudah terkirim**: baris dikembalikan ke `pending` dengan hitungan percobaan nol, dirender ulang dari payload-nya dengan brand dan SMTP yang berlaku saat itu, ke penerima yang sama; baris `sending` (sedang dipegang worker) tidak disentuh. Di web, kirim ulang **selalu dikonfirmasi dulu** (pola `ConfirmDelete`, token `confirm=resend`): tanpa JavaScript lewat langkah halaman `?confirm=resend&id=`, dengan JavaScript lewat modal yang mem-POST via fetch; form action menolak POST tanpa token itu. |
...
| L-10 | **[P0]** **Tema baku sistem** ditetapkan admin dari konfigurasi database (`app.default_theme`, per tenant dengan fallback global). Ini yang dilihat pengunjung anonim dan user yang belum memilih. **Nilai awal (belum diubah admin): `warm`** — juga tema bawaan di ujung rantai L-12. |
...
| O-3 | **[P0]** Seed idempoten: superadmin pertama, tenant baku, group baku (Administrator dengan `*.*`; Regular User **tanpa izin apa pun** — pengguna biasa tidak boleh melihat daftar pengguna, mis. saat aplikasi dipakai untuk e-commerce; admin menambah izin sesuai kebutuhan deployment), daftar izin, konfigurasi awal (termasuk tema & landing route baku), dan data contoh untuk modul `Example`. Seed hanya **menambah** izin yang belum ada, jadi mengubah daftar bakunya tidak menyentuh database yang sudah di-seed. |
