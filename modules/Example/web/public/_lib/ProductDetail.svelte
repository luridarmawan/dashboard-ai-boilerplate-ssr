...
/**
 * One product, with its per-page SEO (title, description, canonical, Open Graph, JSON-LD — R-4).
 * Shared by `/product/[slug]` and by the 404 handler `/resolve` when a root URL names a product
 * (F-11): the same markup, only the canonical URL and the breadcrumb differ.
 */
interface Product {
  slug: string;
  name: string;
...
let {
  ...
  product: Product;
  brand: string;
  /** The canonical, absolute URL of this rendering. */
  url: string;
  /** Extra breadcrumb entries between the brand and the product (e.g. its category). */
  crumbs?: { href: string; label: string }[];
} = $props();
const t = useT();
const locale = useLocale();
const p = $derived(product);
...
<svelte:head>
  <title>{p.name} — {brand}</title>
  <meta name="description" content={desc} />
  <link rel="canonical" href={url} />
  <meta property="og:type" content="product" />
  <meta property="og:title" content={`${p.name} — ${brand}`} />
...
<article class="mx-auto grid max-w-6xl gap-12 px-4 py-12 md:grid-cols-2 md:items-start">
  <div class="relative aspect-[4/5] w-full overflow-hidden rounded-3xl border bg-gradient-to-br from-primary/30 via-primary/10 to-accent shadow-sm">
    {#if p.imageUrl}
      <!-- The one image above the fold here: `priority` keeps it eager and asks to be fetched
           early, because `loading="lazy"` on the LCP element delays the very thing the page is
           judged on (PRD §8). Every other image in this module is lazy by default. -->
      <Img src={p.imageUrl} alt={p.name} priority width={800} height={1000} class="h-full w-full object-cover" />
    {:else}
  ...
  <div class="md:sticky md:top-24">
    <nav class="text-sm text-muted-foreground" aria-label="Breadcrumb">
      <a href="/example" class="hover:text-foreground">{brand}</a>
      {#each crumbs as c (c.href)}
        <span aria-hidden="true">/</span> <a href={c.href} class="hover:text-foreground">{c.label}</a>
      {/each}
      <span aria-hidden="true">/</span> <a href="/example#products" class="hover:text-foreground">{t('example.products.title')}</a>
    </nav>
