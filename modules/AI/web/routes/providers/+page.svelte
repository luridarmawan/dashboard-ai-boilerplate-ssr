...
<div class="page">
  <div class="flex flex-wrap items-center justify-between gap-3">
    ...
    {#if manage}<Button href="/m/ai/providers/new" size="sm"><Icon name="plus" size={16} />{t('ai.providers.new')}</Button>{/if}
  </div>
      ...
      {#each data.providers as p (p.id)}
        <tr data-testid="provider-row">
          ...
          <td>
            ...
            <span class="ms-1 text-xs text-muted-foreground">{fmt(p.lastTestedAt)}</span>
          </td>
          <td class="text-end whitespace-nowrap"><a href={`/m/ai/providers/${p.id}`} title={rowAction} aria-label={rowAction} class={iconActionClass}><Icon name={manage ? 'edit' : 'eye'} size={16} /></a></td>
        </tr>
      {:else}
