...
<div class="page">
  ...
  <Table caption={t('ai.logs.title')}>
    <thead><tr><th>Waktu</th><th>{t('ai.logs.model')}</th><th class="text-right">{t('ai.logs.tokens')}</th><th class="text-right">{t('ai.logs.latency')}</th><th>{t('ai.logs.status')}</th><th class="text-right">{t('ai.logs.cost')}</th></tr></thead>
    <tbody>
      {#each data.logs as l (l.id)}
        <tr>
          ...
          <td class="text-right">{l.costMicro === null ? '—' : (l.costMicro / 1_000_000).toFixed(4)}</td>
        </tr>
      {:else}
