 ...
 * Accept-Language → configured default (`LOCALE_DEFAULT` until M3's per-tenant configuration).
...
export function resolveRequestLocale(event: RequestEvent): LocaleResolution {
  return resolveLocale({
    user: event.locals.session?.user.locale ?? null,
    cookie: event.cookies.get(LANG_COOKIE) ?? null,
    acceptLanguage: event.request.headers.get('accept-language'),
    defaultLocale: env.LOCALE_DEFAULT ?? null,
  });
}
