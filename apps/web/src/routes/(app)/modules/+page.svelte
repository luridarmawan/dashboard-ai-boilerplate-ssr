...
const chips = (c: Mod['contributes']): [string, number, string][] =>
  (
    [
      ['grid', c.tables, t('modules.chip.tables')],
      ...
      ['dashboard', c.widgets, t('modules.chip.widgets')],
...
const sourceLabel = (s: string) =>
  s === 'local' ? t('modules.source_local') : s === 'submodule' ? t('modules.source_submodule') : s;
...
const statusBadge = (s: string): [string, 'success' | 'secondary' | 'outline' | 'destructive'] =>
  s === 'installed'
    ? [t('modules.installed'), 'success']
    : s === 'update'
      ? [t('modules.catalog.status_update'), 'secondary']
      : s === 'incompatible'
        ? [t('modules.catalog.status_incompatible'), 'destructive']
        : [t('modules.catalog.status_available'), 'outline'];
...
<svelte:head><title>{t('nav.modules')}</title></svelte:head>

<div class="page">
  <div class="flex flex-wrap items-start justify-between gap-3">
    <div>
      <h1>{t('nav.modules')}</h1>
      <p class="mt-1 max-w-3xl text-sm text-muted-foreground">{t('modules.lead')}</p>
    </div>
    <dl class="grid grid-cols-3 gap-2 text-center">
      <div class="rounded-lg border bg-card px-3 py-2"><dt class="text-[10px] uppercase tracking-wide text-muted-foreground">{t('modules.installed')}</dt><dd class="text-xl font-semibold">{total}</dd></div>
      <div class="rounded-lg border bg-card px-3 py-2"><dt class="text-[10px] uppercase tracking-wide text-muted-foreground">{t('common.active')}</dt><dd class="text-xl font-semibold text-success">{active}</dd></div>
      <div class="rounded-lg border bg-card px-3 py-2"><dt class="text-[10px] uppercase tracking-wide text-muted-foreground">{t('modules.unhealthy')}</dt><dd class={`text-xl font-semibold ${unhealthy ? 'text-destructive' : ''}`}>{unhealthy}</dd></div>
    </dl>
  </div>
      ...
      <Card class={m.effective ? '' : 'opacity-80'}>
        <div class="flex items-start gap-3">
          ...
          <div class="min-w-0 flex-1">
            <div class="flex flex-wrap items-center gap-2">
              ...
              <Badge variant={m.effective ? 'success' : 'destructive'}>{m.effective ? t('common.active') : t('common.inactive')}</Badge>
              {#if m.health.status === 'failed'}<Badge variant="destructive"><Icon name="warning" size={12} class="me-1" />{t('modules.unhealthy')}</Badge>{:else if m.health.status === 'ok'}<Badge variant="secondary"><Icon name="check" size={12} class="me-1" />{t('modules.healthy')}</Badge>{/if}
            </div>
            ...
            {#if can('module.manage')}
              ...
              <p class="mt-1 text-xs text-muted-foreground">{t('modules.remove')} <code>bun modules:remove {m.name}</code> <span title={t('modules.remove_title')}>{t('modules.remove_note')}</span></p>
            {/if}
          </div>
        </div>

        <ul class="mt-4 flex flex-wrap gap-1.5" aria-label={t('modules.contributions')}>
        ...
        <div class="mt-4 grid gap-3 border-t pt-4 sm:grid-cols-2">
          <div class="grid gap-2">
            <p class="text-[10px] uppercase tracking-wide text-muted-foreground">{t('modules.scope_tenant')}</p>
            <div class="flex flex-wrap items-center gap-2">
              {#if m.tenantEnabled === null}<Badge variant="outline">{t('modules.inherit_global')}</Badge>{:else}<Badge variant={m.tenantEnabled ? 'success' : 'secondary'}>{m.tenantEnabled ? t('common.active') : t('common.inactive')}</Badge>{/if}
              {#if can('module.manage')}
                <form method="POST" action="?/toggle" class="flex flex-wrap gap-1">
                  ...
                  <Button type="submit" name="state" value={m.effective ? 'off' : 'on'} size="sm" variant={m.effective ? 'outline' : 'default'}>{m.effective ? t('modules.disable') : t('modules.enable')}</Button>
                  {#if m.tenantEnabled !== null}<Button type="submit" name="state" value="inherit" size="sm" variant="ghost">{t('modules.follow_global')}</Button>{/if}
                </form>
              {/if}
            </div>
          </div>
          <div class="grid gap-2">
            <p class="text-[10px] uppercase tracking-wide text-muted-foreground">{t('modules.scope_global')}</p>
            <div class="flex flex-wrap items-center gap-2">
              <Badge variant={m.globalEnabled ? 'success' : 'secondary'}>{m.globalEnabled ? t('common.active') : t('common.inactive')}</Badge>
              {#if data.canGlobal}
                <form method="POST" action="?/toggle">
                  <Csrf token={data.csrf} />
                  <input type="hidden" name="module" value={m.name} />
                  <input type="hidden" name="scope" value="global" />
                  <Button type="submit" name="state" value={m.globalEnabled ? 'off' : 'on'} size="sm" variant="ghost">{m.globalEnabled ? t('modules.disable_global') : t('modules.enable_global')}</Button>
                </form>
              {/if}
            </div>
          </div>
        ...
        {#if m.health.jobs.length || m.health.hookFailures.length}
          <details class="mt-4 rounded-md border bg-muted/40 text-sm" open={m.health.status === 'failed'}>
            <summary class="cursor-pointer px-3 py-2 text-muted-foreground">{t('modules.health_summary', { jobs: m.health.jobs.length, hooks: m.health.hookFailures.length })}</summary>
            <div class="grid gap-2 px-3 pb-3">
              {#each m.health.jobs as j (j.name)}
                <div class="flex flex-wrap items-center gap-2">
                  ...
                  {#if j.lastStatus === 'ok'}<Badge variant="success">ok</Badge>{:else if j.lastStatus === 'failed'}<Badge variant="destructive">{t('modules.job_failed')}</Badge>{:else}<Badge variant="outline">{t('modules.job_never')}</Badge>{/if}
  ...
  {#if data.catalog}
    ...
    <section class="mt-8" data-testid="module-catalog">
      <div class="flex flex-wrap items-end justify-between gap-2">
        <div>
          <h2>{t('modules.catalog.title')}</h2>
          <p class="mt-1 max-w-3xl text-sm text-muted-foreground">{t('modules.catalog.lead_before')}<code>bun modules:install &lt;Nama&gt;</code>{t('modules.catalog.lead_after')} {t('modules.catalog.source')} {data.catalog.source === 'url' ? data.catalog.url : data.catalog.source === 'file' ? 'modules.catalog.json' : t('modules.catalog.source_none')}{#if data.catalog.coreVersion} · core v{data.catalog.coreVersion}{/if}.</p>
        </div>
      </div>
      {#if data.catalog.error}<p class="error mt-3" role="alert">{t('modules.catalog.error', { error: data.catalog.error })}</p>{/if}
        ...
        {#each catalogEntries as e (e.name)}
          ...
          <Card class={e.status === 'incompatible' ? 'opacity-70' : ''}>
            <div class="flex flex-wrap items-center gap-2">
              ...
              <Badge {variant}>{label}{#if e.status === 'update'} {t('modules.catalog.from_version', { version: e.installedVersion ?? '' })}{/if}</Badge>
              {#if e.bundled}<Badge variant="secondary">{t('modules.catalog.bundled')}</Badge>{/if}
            </div>
            ...
            <p class="mt-2 text-xs text-muted-foreground">{t('modules.catalog.requires_core')} <code>{e.engines.core}</code>{#if e.homepage} · <a href={e.homepage} rel="noopener" target="_blank">{t('modules.catalog.homepage')}</a>{/if}</p>
            {#if e.installCommand}
