...
const groups: [string, string, readonly string[]][] = [
  [
    'Permukaan',
    'Latar halaman, kartu, dan popover beserta teks di atasnya.',
    ['background', 'foreground', 'card', 'card-foreground', 'popover', 'popover-foreground'],
  ],
  [
    'Aksi',
    'Tombol utama, sekunder, dan aksen (tautan, sorotan).',
    [
      'primary',
      'primary-foreground',
      'secondary',
  ...
  ['Peredam', 'Area sekunder dan teks keterangan.', ['muted', 'muted-foreground']],
  [
    'Status',
    'Merah/hijau/kuning untuk galat, sukses, dan peringatan.',
    [
      'destructive',
  ...
  [
    'Garis & fokus',
    '--border dekoratif; --input dan --ring wajib ≥ 3:1 terhadap latar.',
    ['border', 'input', 'ring'],
  ],
  ['Bagan', 'Lima warna deret untuk chart.', COLOR_TOKENS.filter((n) => n.startsWith('chart-'))],
];
...
<form method="POST" {action} enctype="multipart/form-data" class="grid gap-4 lg:grid-cols-[minmax(0,1fr)_400px] lg:items-start">
  ...
  <div class="grid gap-4">
    ...
    <Card title="Identitas" description="Nama dua bahasa, slug untuk id tema, dan dari tema mana token-nya disalin.">
    ...
    <Card title="Layout per jenis shell" description="Hanya layout terdaftar (core atau modul). Varian yang dikosongkan mengikuti default (Keputusan K).">
      <div class="grid gap-3 md:grid-cols-3">
        {#each KINDS as kind (kind)}
          <fieldset class="grid gap-2">
            <legend class="px-1 text-sm font-medium">{kindLabel[kind] ?? kind}</legend>
            {#each VARIANTS[kind] as variant (variant)}
              <label class={label}><span class="flex items-center gap-1"><code>{variant}</code>{#if variant === 'default'}<span class="text-xs text-muted-foreground">wajib</span>{/if}</span>
                <select name={`layout__${kind}__${variant}`} class={input} disabled={readonly}>
                  ...
                  {#each layoutsOf(kind) as l (l.id)}<option value={l.id} selected={values.layouts[kind]?.[variant] === l.id}>{l.name.id}</option>{/each}
                </select>
              </label>
            {/each}
    ...
    <Card title="Bentuk & huruf" description="Radius sudut dan tumpukan font. Font harus tersedia di klien atau di-self-host.">
          ...
          {#each names as name (name)}
            ...
            <div class="grid gap-2 rounded-md border p-2.5 text-sm" data-token={name}>
              <div class="flex items-center justify-between gap-2">
                ...
                {#if report.some((r) => !r.ok && (r.fg === name || r.bg === name))}<Badge variant="destructive">kontras</Badge>{/if}
              </div>
              <div class="grid grid-cols-2 gap-2">
                <label class="flex items-center gap-2 rounded-md border bg-background px-2 py-1" title="mode terang">
                  ...
                  <span class="grid leading-tight"><span class="text-[10px] uppercase tracking-wide text-muted-foreground">terang</span><span class="font-mono text-xs">{light[name] ?? '—'}</span></span>
                </label>
                <label class="flex items-center gap-2 rounded-md border bg-background px-2 py-1" title="mode gelap">
                  ...
                  <span class="grid leading-tight"><span class="text-[10px] uppercase tracking-wide text-muted-foreground">gelap</span><span class="font-mono text-xs">{dark[name] ?? light[name] ?? '—'}</span></span>
                </label>
              </div>
    ...
    {#if !readonly}
      <div class="flex flex-wrap items-center gap-2">
        <Button type="submit" size="lg"><Icon name="save" size={16} />{isNew ? 'Buat tema' : 'Simpan perubahan'}</Button>
        <Button href="/themes" variant="outline" size="lg">Kembali</Button>
        <span class="text-sm text-muted-foreground">Disimpan hanya bila semua pasangan kontras lolos AA.</span>
      </div>
    {/if}
  </div>
  ...
  <aside class="grid gap-4 lg:sticky lg:top-20">
    <Card title="Pratinjau langsung" description="Komponen asli dengan token yang sedang Anda ubah — bukan gambar.">
      <div class="grid gap-3">
        {#each [['light', 'Terang', lightStyle], ['dark', 'Gelap', darkStyle]] as [mode, name, style] (mode)}
          <div class="overflow-hidden rounded-lg border" data-mode={mode} data-testid={`live-preview-${mode}`}>
            <div class="bg-background p-3 text-foreground" {style}>
              <div class="mb-2 flex items-center justify-between text-xs">
                ...
                <span class="inline-flex items-center gap-1 rounded-md bg-primary px-2 py-0.5 text-primary-foreground"><Icon name="sparkles" size={12} />{values.nameId || 'Tema'}</span>
              </div>
              <div class="rounded-md border bg-card p-3 text-card-foreground shadow-xs">
                <p class="text-sm font-semibold">Judul kartu</p>
                <p class="text-xs text-muted-foreground">Teks keterangan di atas kartu.</p>
                <div class="mt-2 flex flex-wrap gap-1.5">
                  <span class="inline-flex h-7 items-center rounded-md bg-primary px-2.5 text-xs font-medium text-primary-foreground">Utama</span>
                  <span class="inline-flex h-7 items-center rounded-md bg-secondary px-2.5 text-xs font-medium text-secondary-foreground">Sekunder</span>
                  ...
                  <span class="inline-flex h-7 items-center rounded-md bg-destructive px-2.5 text-xs font-medium text-destructive-foreground">Hapus</span>
                </div>
                <div class="mt-2 flex flex-wrap gap-1.5">
                  <span class="rounded-md bg-success px-2 py-0.5 text-xs text-success-foreground">sukses</span>
                  <span class="rounded-md bg-warning px-2 py-0.5 text-xs text-warning-foreground">peringatan</span>
                  <span class="rounded-md bg-accent px-2 py-0.5 text-xs text-accent-foreground">aksen</span>
                </div>
                <div class="mt-2 h-8 rounded-md border border-input bg-background px-2 text-xs leading-8 text-muted-foreground ring-2 ring-ring/40">Kolom isian dengan cincin fokus</div>
    ...
    <Card title="Kontras WCAG AA (L-21)" description="Setiap pasangan diperiksa di kedua mode; 4.5 untuk teks, 3.0 untuk komponen.">
      <div class="mb-2 flex items-center gap-2 text-sm">
        {#if failing}<Badge variant="destructive">{failing} pasangan gagal</Badge>{:else}<Badge variant="success">semua lolos</Badge>{/if}
        {#if contrast.length && failing === 0}<span class="text-xs text-muted-foreground">(hasil server sebelumnya: {contrast.length} gagal)</span>{/if}
      </div>
