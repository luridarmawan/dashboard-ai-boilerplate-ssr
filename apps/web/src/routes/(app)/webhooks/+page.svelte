    ...
    <tbody>
      {#each data.webhooks as w (w.id)}
        <tr data-testid="webhook-row">
          ...
          <td>{#each w.events as e (e)}<code class="mr-1">{e}</code>{/each}</td>
          <td>
            ...
            <span class="ml-1 text-xs text-muted-foreground">{fmt(w.lastDeliveredAt)}</span>
          </td>
          <td class="text-right"><a href={`/webhooks/${w.id}`}>{can('webhook.manage') ? 'Kelola' : 'Lihat'}</a></td>
        </tr>
      {:else}
